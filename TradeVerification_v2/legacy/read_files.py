...
def read_oms_futures_file():
    # 1. 엑셀 파일 불러오기
    file_path = 'Z:/02.펀드/019. 일간매매내역/test/파생/2024-12-16_futures.xlsx'  # 파일 경로를 적절히 수정하세요.
...
def read_oms_stock_file():
    # 1. 엑셀 파일 불러오기
    file_path = 'Z:/02.펀드/019. 일간매매내역/test/주식/2024-12-16_stock.xlsx'  # 파일 경로를 적절히 수정하세요.
    df = pd.read_excel(file_path)
...
def read_trader_file():
    ...
    # 1. 엑셀 파일 불러오기
    file_path = 'Z:/02.펀드/019. 일간매매내역/test/12월16일 전체.xlsx'  # 파일 경로를 적절히 수정하세요.
    df = pd.read_excel(file_path)
...
def merge_and_reconcile():
    ...
    oms_combined = pd.concat([oms_stock_df, oms_futures_df], ignore_index=True)
    aggregated_oms = aggregate_by_key(oms_combined)
    ...
    # 결과 출력
    print("대사 결과:")
    print(reconciliation_df[['펀드명', '매매구분', '단축코드', '체결단가_차이', '체결수량_차이', '체결금액_차이']])

    # 결과를 파일로 저장 (옵션)
    reconciliation_df.to_excel("reconciliation_result.xlsx", index=False)
